        ...
        with cols[1]:
            ...
            with st.popover('Select imputation strategy'):
                with st.form('Missing value imputation'):
                    ...
                    impute_strategies = ('mean','most_frequent','median', 'constant')
                    st.write('')
                    for col in missing_val_cols:
                        form_cols = st.columns([2,6])
                        with form_cols[0]:
                            st.write('Feature')
                            st.markdown(f'&nbsp;&nbsp;&nbsp;<b>{col} </b>', unsafe_allow_html=True)
                        with form_cols[1]:
                            impute_strategy=st.selectbox('Impute method',impute_strategies, key=col)
                            st.session_state[f'{col}_impute_strategy'] = impute_strategy
                            if impute_strategy == 'constant':
                                st.session_state['impute_fill_constant'] = st.number_input('Type number to fill missing values')
                            else:
                                pass
